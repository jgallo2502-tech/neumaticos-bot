...
function normalizarMedida(texto) {
  ...
  // Formato sin perfil métrico: 195 R14, 165 HR14, 175 HR13 (anchos 1xx-3xx, letra velocidad opcional)
  const m3 = t.match(/([123]\d{2})\s*(?:[A-Z]\s*)?[rR]\s*(\d{2})(C)?\b/i);
  if (m3) return `${m3[1]}R${m3[2]}${m3[3] ? 'C' : ''}`;
  // Formato clásico con punto: 6.40 SR13, 7.25 SR13, 5.00R12 (letra velocidad opcional)
  const mClas = t.match(/(\d)\s*[.,]\s*(\d{2})\s*(?:[A-Z]\s*)?[rR]\s*(\d{2})(C)?\b/i);
